...
One should always set a background-color as backup in case if an image is not available.
...
There are a couple of ways to add a background-image. One is to set the background-image targeting the body element. Something like this:
...
First I set the background-image property to the value of `url(/images/bryan-minear-317365.jpg);`. The path I use here is the absolute path to the image in the project folder. However, locally you can use a relative path if you like. But it is better to set an absolute path and then keep it for remote. On remote, it should be an absolute path.
...
Lastly, I set the `overflow-x` property to the value of `hidden;`. This means that if the width of the body overflows the x axis of the viewport, setting the value of hidden to the `overlow-x` property hides the scrollbar that appears when there is an overflow on the x-axis.
...
We don't have to always set a background-image to the body of a web page. We can set it to another element. In the example below, I set it to the background of a section element with the class of `.quote-container`. This time, however, I set the CSS styles via JavaScript. The same can be done with plain CSS. Set with plain CSS, it would look like the following:
...
However, I can only set one background-image. I can set a number of background images just in case if any previous are not available, but those are only fallbacks.

Using JavaScript, I can dynamically set multiple background images on the click of the section element with the class of `.quote-container`, dynamically changing the background-image each time I click on the section element.

I also set the CSS styles via JavaScript. This is also known as CSS in JS.
...
Using a template string formed with back ticks `(``)`, I set the url of the background-image to dynamically be `images/` + a random image taken from a const variable called images with the value of an array of images.
...
Each path within a template string represents the name of an image file with the `.jpg` extension. All these images reside within a folder called images within the root of my Portfolio Site folder, but because I have to separately add the folder name to the image name in order to be able to render it in the browser, I also have to declare and assign a variable to the value of the name of the directory:
...
Next, I needed to set the addEventListener() method on the quoteContainer. This method sets up a function that will be called whenever the specified event is delivered to the target. In our case here, the addEventListener() method is attaching an event handler to a specified element, which is the section element with the class `.quote-container`.
...
The articleHeading's content is cleared by setting the .innerHTML JavaScript property to an empty template string. If you recall, by default, the articleHeading contains the text content of `Click for quote!`. In order to be able to replace that text with the text of a random quote from the quotes array, we first have to set the quoteCloud's innerHTML to the empty template string. Otherwise, it would remain.

Next, I set the .textContent property on the blockQuote variable to the value of `quotes[randomIndex]`. `quotes` represents the variable containing the array of .jpg images as its value, and `[randomIndex]` represents the random index of the quotes array, generated with the `Math.floor(Math.random() * quotes.length)` method. The Math.floor() method rounds down a number containing decimals to its largest integer, less than or equal to the number.
...
`quotes.length` represents the "length" of the quotes array. In other words, the number of quotes inside the quotes array, each surrounded by template strings. In our quotes array, we have 11 quotes.
...
Next, I set the .style property to the quoteCloud variable using the camel-cased backgroundPosition property to set the position of the background-image.
...
I use a template string to wrap around the value of `quoteCloud.style.backgroundPosition`, but I also could have used single or double quotes, One just has to be consistent when using single or double quotes. Its industry convention and standard to keep things stylistically consistent.
...
Next, I set the .style property to the quoteCloud variable using the camel-cased backgroundSize property to set the size of the background-image.
...
Next, I set the .style property on the quoteCloud variable with the class of "quote-cloud" in the HTML markup, and using the height property to set the fluid height of the article element with the class of "quote-cloud".
...
Next, I set the .style property on the quoteCloud variable with the class of "quote-cloud" in the HTML markup, and using the camel-cased maxHeight property to set the maximum height permissible of the article element with the class of "quote-cloud".
...
Last of all, I set the .style property on the quoteCloud variable with the class of"quote-cloud" in the HTML markup, and using the outline property to set a double border sitting right outside of the article element with the class of "quote-cloud" in the HTML markup, to set a double outline that is drawn around article element, outside the borders, instead of the inside edges, to make the element "stand out".
...
Instead, we are setting the JavaScript .style property on the quoteCloud variable with the class of "quote-cloud" in the HTML markup, and using the outline property to set a double outline drawn around article element, outside the borders, instead of the inside edges, to make the element "stand out".
...
I do the same with dynamically setting the backgroundColor of the body element via the const variable called body.
...
I set CSS in JS that was related specifically to the background-image of the article element with the class of `.quote-cloud`. As for the placement and styling of the blockquote element, and other more general styling which did not have to be applied via JavaScript, I defined those styles inside my exgternal stylesheet:
